...
/**
 * 代理：转发到 app/backend 的 GET /internal/users/:address/ledger/entries
 * Query: address, asset, bizType, cursor, limit
 *
...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const key of ['asset', 'bizType', 'cursor', 'limit']) {
    ...
    const url = `${base.replace(/\/$/, '')}/internal/users/${encodeURIComponent(
      address.toLowerCase(),
    )}/ledger/entries?${forward.toString()}`;
